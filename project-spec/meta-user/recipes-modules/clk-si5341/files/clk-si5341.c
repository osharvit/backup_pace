	...
	u64 *val1, u32 *val2)
{
	...
	if (err < 0) {
		printk(KERN_ERR "si5341_decode_44_32 regmap_bulk_read err = %d\n", err);
		return err;
	}

	printk(KERN_ERR "si5341_decode_44_32 regmap_bulk_read r[4] = %X; r[5] = %X; r[6] = %X\n", r[4], r[5], r[6]);
	...
	u64 n_num, u32 n_den)
{
	...
	/* Program the fraction */
        printk(KERN_ERR "regmap_bulk_write: to %04X\n", reg);
        for(i = 0; i < 10; i++)
        	printk(KERN_ERR "  %0X", r[i]); 
        printk(KERN_ERR "\n");
		...
		unsigned long parent_rate)
{
	...
	u32 m_den;
	unsigned int shift;
	printk(KERN_ERR "si5341_clk_recalc_rate; parent_rate = %lu\n", parent_rate);
...
static int si5341_clk_get_selected_input(struct clk_si5341 *data)
{
	...
	if (err < 0) {
		printk(KERN_ERR "si5341_clk_get_selected_input; err = %d\n", err);
		return err;
	}
...
static u8 si5341_clk_get_parent(struct clk_hw *hw)
{
	...
	printk(KERN_ERR "si5341_clk_get_parent entered\n");
	printk(KERN_ERR "si5341_clk_get_parent si5341_clk_get_selected_input = %d\n",res);
	if (res < 0) {
...
static int si5341_clk_reparent(struct clk_si5341 *data, u8 index)
{
	int err;
	u8 val;
        printk(KERN_ERR "si5341_clk_reparent; index = %d\n", index);
	...
	printk(KERN_ERR "si5341_clk_reparent finished\n");
	return 0;
}
...
static int si5341_synth_clk_is_on(struct clk_hw *hw)
{
	...
	printk(KERN_ERR "si5341_synth_clk_is_on starteed\n");
	...
	printk(KERN_ERR "si5341_synth_clk_is_on finished\n");
...
static int si5341_synth_clk_prepare(struct clk_hw *hw)
{
	...
	u8 index = synth->index;
	u8 mask = BIT(index);
	printk(KERN_ERR "si5341_synth_clk_prepare entered\n");
	/* Power up */
		...
		unsigned long parent_rate)
{
	...
	printk(KERN_ERR "si5341_synth_clk_recalc_rate synth->index = %d\n", synth->index);
	err = si5341_decode_44_32(synth->data->regmap,
			SI5341_SYNTH_N_NUM(synth->index), &n_num, &n_den);
	if (err < 0) {
		printk(KERN_ERR "si5341_synth_clk_recalc_rate , err = %d\n", err);
		return err;
	}

	printk(KERN_ERR "si5341_synth_clk_recalc_rate; n_num = %llu; n_den = %u\n", n_num, n_den);
		...
		unsigned long *parent_rate)
{
	struct clk_si5341_synth *synth = to_clk_si5341_synth(hw);
	u64 f;
	printk(KERN_ERR "si5341_synth_clk_round_rate rate = %lu; *parent_rate = %lu\n", rate, *parent_rate);
	/* The synthesizer accuracy is such that anything in range will work */
	f = synth->data->freq_vco;
	do_div(f, SI5341_SYNTH_N_MAX);
	if (rate < f) {
	printk(KERN_ERR "si5341_synth_clk_round_rate (1) = %llu", f);
		return f;
	}
	f = synth->data->freq_vco;
	do_div(f, SI5341_SYNTH_N_MIN);
	if (rate > f) {
		printk(KERN_ERR "si5341_synth_clk_round_rate (2) = %llu", f);
		return f;
	}
	printk(KERN_ERR "si5341_synth_clk_round_rate (3) = %lu", rate);
	return rate;
}
	...
	u64 n_num, u32 n_den, bool is_integer)
{
	int err;
	u8 index = synth->index;
	printk(KERN_ERR "si5341_synth_program  entered\n");
	...
	if (err < 0) {
		printk(KERN_ERR "si5341_synth_program ; err = %d\n", err);
		return err;
	}
		...
		unsigned long parent_rate)
{
	...
	u32 g;
	bool is_integer;
	printk(KERN_ERR "si5341_synth_clk_set_rate; rate = %lu; parent_rate = %lu\n",rate, parent_rate);
	...
	dev_dbg(&synth->data->i2c_client->dev,
			"%s(%u): n=0x%llx d=0x%x %s\n", __func__,
				...
				is_integer ? "int" : "frac");
        printk(KERN_ERR "%s(%u): n=0x%llx d=0x%x %s\n", __func__,
                                synth->index, n_num, n_den,
                                is_integer ? "int" : "frac");
...
static int si5341_output_clk_is_on(struct clk_hw *hw)
{
	...
	int err;
	u32 val;
	printk(KERN_ERR "si5341_output_clk_is_on entered\n");
	err = regmap_read(output->data->regmap,
			SI5341_OUT_CONFIG(output), &val);
	if (err < 0) {
		printk(KERN_ERR "si5341_output_clk_is_on err = %d\n", err);
		return err;
	}
...
{
	struct clk_si5341_output *output = to_clk_si5341_output(hw);
	printk(KERN_ERR "si5341_output_clk_unprepare entered\n");
...
static int si5341_output_clk_prepare(struct clk_hw *hw)
{
	struct clk_si5341_output *output = to_clk_si5341_output(hw);
	int err;
	printk(KERN_ERR "si5341_output_clk_prepare entered\n");
	err = regmap_update_bits(output->data->regmap,
			SI5341_OUT_CONFIG(output),
			SI5341_OUT_CFG_PDN, 0);
	if (err < 0) {
		printk(KERN_ERR "si5341_output_clk_prepare err = %d\n", err);
		return err;
	}
		...
		unsigned long parent_rate)
{
	...
	err = regmap_read(output->data->regmap,
			SI5341_OUT_CONFIG(output), &val);
	if (err < 0) {
		printk(KERN_ERR "si5341_output_clk_recalc_rate err = %d\n", err);
		return err;
	}
		...
		unsigned long *parent_rate)
{
	...
	printk(KERN_ERR "si5341_output_clk_round_rate rate = %lu; *parent_rate = %lu\n", rate, *parent_rate);
	if (!rate) {
		return 0;
	}
	...
	if (r && !(r % rate)) {
		printk(KERN_ERR "si5341_output_clk_round_rate 1 rate = %lu\n", rate);
		return (long)rate;
	}
	...
	printk(KERN_ERR "si5341_output_clk_round_rate 2 rate = %lu\n", rate);
	return rate;
}
		...
		unsigned long parent_rate)
{
	...
	int err;
	u8 r[3];
        printk(KERN_ERR "si5341_output_clk_set_rate started; rate = %lu; parent_rate= %lu\n", rate, parent_rate);
	if (!rate) {
           printk(KERN_ERR "si5341_output_clk_set_rate; -EINVAL\n");
		return -EINVAL;
        }
	...
	if (err < 0) {
        	printk(KERN_ERR "regmap_update_bits; err = %d\n", err);
		return err;
	}
	...
	err = regmap_bulk_write(output->data->regmap,
			SI5341_OUT_R_REG(output), r, 3);
        printk(KERN_ERR "reg = %X; r[0] = %X r[1] = %X; r[2] = %X\n", SI5341_OUT_R_REG(output), r[0], r[1], r[2]);
        if (err < 0) {
                printk(KERN_ERR "regmap_bulk_write; err = %d\n", err);
                return err;
        }
	return 0;
}
...
static int si5341_probe_chip_id(struct clk_si5341 *data)
{
	...
	case 0x5341:
		...
		dev_err(&data->i2c_client->dev, "Detected 0x5341\n");
		break;
	case 0x5342:
			      ...
			      struct clk_si5341_output_config *config)
{
	...
	for_each_child_of_node(np, child) {
		...
		dev_err(&data->i2c_client->dev, "Read from DT clkout: %d\n", num);
		...
		const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "si5341_probe started 3\n");
	/* Must be done before otherwise touching hardware */
	err = si5341_wait_device_ready(client);
        dev_err(&client->dev, "si5341_wait_device_ready: err = %d\n", err);
	if (err)
		return err;
	...
	for (i = 0; i < SI5341_NUM_INPUTS; ++i) {
		...
		if (IS_ERR(input)) {
			if (PTR_ERR(input) == -EPROBE_DEFER) {
				dev_err(&client->dev, "devm_clk_get error= -EPROBE_DEFER\n");
				return -EPROBE_DEFER;
                        }
	...
	for (i = 0; i < SI5341_MAX_NUM_OUTPUTS; ++i) {
		...
		snprintf(reg_name, sizeof(reg_name), "vddo%d", i);
                dev_err(&client->dev, "probing devm_regulator_get_optional for: vddo%d\n", i);
	...
	if (err) {
                dev_err(&client->dev, "error si5341_dt_parse_dt: %d\n", err);
		goto cleanup;
        }
	...
	if (IS_ERR(data->regmap)) {
		err = PTR_ERR(data->regmap);
                dev_err(&client->dev, "error devm_regmap_init_i2c: %d\n", err);
		goto cleanup;
	}
	...
	if (err < 0)
		goto cleanup;
        dev_err(&client->dev, "si5341_probe_chip_id returned: %d\n", err);
#if 0
	...
	if (1 /*initialization_required*/) {
		...
		if (err < 0)
			goto cleanup;
                dev_err(&client->dev, "initialization_required - done;\n");
	}
	...
	if (0 /*initialization_required*/) {
		/* PLL configuration is required */
		err = si5341_initialize_pll(data);
		if (err < 0)
			goto cleanup;
                dev_err(&client->dev, "si5341_initialize_pll - done;\n");
	}
	...
	for (i = 0; i < data->num_outputs; ++i) {
		...
		if (config[i].always_on)
			clk_prepare(data->clk[i].hw.clk);
                dev_err(&client->dev, "devm_clk_hw_register fone for output %d\n", i);
	}
	...
	if (1 /*initialization_required*/) {
		...
		if (err < 0)
			goto cleanup;
           dev_err(&client->dev, "si5341_finalize_defaults done;\n");
	}
...
cleanup:
	...
	dev_err(&client->dev, "error %d; cleanup done\n", err);
	return err;
}
...
static int si5341_remove(struct i2c_client *client)
{
	...
	dev_err(&client->dev, "si5341_remove  done\n");
	return 0;
}
