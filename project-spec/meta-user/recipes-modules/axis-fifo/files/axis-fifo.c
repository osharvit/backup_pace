...
struct axis_fifo {
	...
	struct resource *mem_64; /* physical memory */
	void __iomem *base_addr_64; /* kernel space memory */
	uint64_t fpga_addr_64;
	...
	int is_64_bit_bus_rx; /* if the bus is 64 bit WIDTH */
	int is_64_bit_bus_tx; /* if the bus is 64 bit WIDTH */
...
static ssize_t is_64_bit_bus_rx_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct axis_fifo *fifo = dev_get_drvdata(dev);
	unsigned int read_val;
	...
	read_val = fifo->is_64_bit_bus_rx;
	len =  snprintf(tmp, sizeof(tmp), "0x%x\n", read_val);
	memcpy(buf, tmp, len);
	return len;
}

static DEVICE_ATTR_RO(is_64_bit_bus_rx);

static ssize_t is_64_bit_bus_tx_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct axis_fifo *fifo = dev_get_drvdata(dev);
	unsigned int read_val;
...
static DEVICE_ATTR_RO(is_64_bit_bus_tx);
			      ...
			      size_t len, loff_t *off)
{
	...
	words_available = bytes_available /( (fifo->is_64_bit_bus_rx)?sizeof(u64):sizeof(u32));
	...
	while (words_available > 0) {
		...
		if (fifo->is_64_bit_bus_rx){
		      for (i = 0; i < copy; i++) {
			      tmp_buf_64[i] = ioread64(fifo->base_addr_64 +
						    XLLF_RDFD_OFFSET_64);
		      }
// 		      pr_info( "Read 64 %d addr 0x%p\n",copy,(fifo->base_addr_64 +
// 						    XLLF_RDFD_OFFSET_64));

		      if (copy_to_user(buf + copied * sizeof(u64), tmp_buf_64,
				      copy * sizeof(u64))) {
			      reset_ip_core(fifo);
			      return -EFAULT;
		      }
	...
	if (fifo->has_tkeep) {
		leftover = bytes_available % ( (fifo->is_64_bit_bus_rx)?sizeof(u64):sizeof(u32));
		if (leftover) {
		      if(fifo->is_64_bit_bus_rx)
			    {
			  tmp_buf_64[0] = ioread64(fifo->base_addr_64 +
						    XLLF_RDFD_OFFSET_64);
			       ...
			       size_t len, loff_t *off)
{
	...
	words_to_write = len / (fifo->is_64_bit_bus_tx?sizeof(u64):sizeof(u32));
        leftover = len % (fifo->is_64_bit_bus_tx?sizeof(u64):sizeof(u32));
	...
	while (words_to_write > 0) {
	  ...
	  if (fifo->is_64_bit_bus_tx){

		  	if (copy_from_user(tmp_buf_64, buf + copied * sizeof(u64),
					copy * sizeof(u64))) {
			      reset_ip_core(fifo);
			      return -EFAULT;
		      }
		      ...
		      for (i = 0; i < copy; i++)
			      iowrite64(tmp_buf_64[i], fifo->base_addr_64 +
					XLLF_TDFD_OFFSET_64);
// 		      pr_info( "write 64 %d addr 0x%p\n",copy,(fifo->base_addr_64 +
// 					XLLF_TDFD_OFFSET_64));      
	...
	if (fifo->has_tkeep && leftover) {
		if(fifo->is_64_bit_bus_tx){
		      ...
		      if (copy_from_user(tmp_buf, buf + copied * sizeof(u64),
					leftover)) {
			      reset_ip_core(fifo);
			      return -EFAULT;
		      }
		      iowrite64(tmp_buf_64[0], fifo->base_addr_64 +
				XLLF_TDFD_OFFSET_64);
	...
	if (fifo->is_64_bit_bus_tx)
	{
	    copiedBytes = (fifo->has_tkeep && !!leftover) ? (copied*sizeof(u64)+leftover) : (copied*sizeof(u64));
	}
	else
	{
...
static void platform_get_print(struct platform_device *dev)
{
	u32 i;

	pr_info("axis-fifo start resource\n");
	for (i = 0; i < dev->num_resources; i++) {
		struct resource *r = &dev->resource[i];
		pr_info("fifo r name %s start %x end %x flags 0x%x\n",r->name,r->start,r->end,r->flags);
	}
//	return NULL;
}
...
static int axis_fifo_probe(struct platform_device *pdev)
{
	...
	{
		tx_max_pkt_size=257;
		pr_info("xlnx,tx-max-pkt-size is not present set to %d\n",tx_max_pkt_size);
		  //goto err_unmap;
	}
	...
	{
  		rx_min_pkt_size=255;
		pr_info("xlnx,tx-min-pkt-size is not present set to %d\n",rx_min_pkt_size);
		/*goto err_unmap;*/
	}
	...
	if ( (rxd_tdata_width != 32) && (rxd_tdata_width != 64) ) {
		dev_err(fifo->dt_device,
	...
	if ((txd_tdata_width != 32) && (txd_tdata_width != 64) )  {
		dev_err(fifo->dt_device,
	...
	if ((rxd_tdata_width==64) ||(txd_tdata_width==64))
	{
	      /* get iospace for the device */
	      ...
	      fifo->mem_64 = r_mem;
	      dev_dbg(fifo->dt_device,"start 0x%p size %0xp\n",fifo->mem_64->start,fifo->mem_64->end);
	      /* request physical memory */
	      if (!request_mem_region(fifo->mem_64->start, resource_size(fifo->mem_64),
				      DRIVER_NAME)) {
		      dev_err(fifo->dt_device,
			      "couldn't lock memory region at 0x%pa\n",
			      &fifo->mem_64->start);
	      ...
	      dev_dbg(fifo->dt_device, "got memory location [0x%pa - 0x%pa]\n",
		      &fifo->mem_64->start, &fifo->mem_64->end);
	      fifo->fpga_addr_64 = fifo->mem_64->start;

	      /* map physical memory to kernel virtual address space */
	      fifo->base_addr_64 = ioremap(fifo->mem_64->start, resource_size(fifo->mem_64));
	      if (!fifo->base_addr_64) {
		      dev_err(fifo->dt_device, "couldn't map physical memory\n");
		      rc = -ENOMEM;
		      goto err_mem_64;
	      }
	      dev_dbg(fifo->dt_device, "remapped memory to 0x%p\n", fifo->base_addr_64);
	      dev_err(fifo->dt_device, "remapped memory to 0x%p\n", fifo->base_addr_64);
	}
	 ...
	 * - data-interface-type
	 */
	fifo->is_64_bit_bus_rx = (rxd_tdata_width==32)?0:1;
	fifo->is_64_bit_bus_tx = (txd_tdata_width==32)?0:1;
	dev_err(fifo->dt_device, "rx_64 %d , tx_64 %d\n",fifo->is_64_bit_bus_rx,fifo->is_64_bit_bus_tx);
...
err_unmap_64:
	iounmap(fifo->base_addr_64);
err_mem_64:
	release_mem_region(fifo->mem->start, resource_size(fifo->mem_64));
err_unmap:
...
static int __init axis_fifo_init(void)
{
	...
	rc =platform_driver_register(&axis_fifo_driver);
	if (rc)
	  pr_info( "platform_driver_register return error = %d\n",rc);
	return rc;
}
