...
static void sync_callback(void *completion)
{
	 ...
	 */
	printk(KERN_ERR "sync_callback before complete\n");
	complete(completion);
	printk(KERN_ERR "sync_callback after complete\n");
}
 ...
 * transaction
 */
static void start_transfer(struct dma_proxy_channel *pchannel_p, int hw_cycle)
{
	 ...
	 */
	if (hw_cycle == 0) {
	/* When using buffer(s) one by one in SW, use this code */
	...
	/* When using cycle on same descriptor, use this code */ 
		chan_desc = dma_device->device_prep_dma_cyclic(pchannel_p->channel_p, pchannel_p->bdtable[bdindex].dma_handle, pchannel_p->buffer_table_p[bdindex].length,
...
/* The following function will call dma_pause and then, stop DMA transfer
 * in xilinx_dma driver
 */
static void stop_transfer(struct dma_proxy_channel *pchannel_p)
{
	struct dma_device *dma_device = pchannel_p->channel_p->device;
	dma_device->device_terminate_all(pchannel_p->channel_p);
	printk(KERN_ERR "DMA stopped\n");
}
...
static void tx_test(struct work_struct *unused)
{
	...
	start_transfer(&channels[TX_CHANNEL], 0);
...
static void test(void)
{
	...
	start_transfer(&channels[RX_CHANNEL], 0);
...
static long ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case START_XFER:
			start_transfer(pchannel_p, 0);
			break;
		case START_XFER_CYCLE:
			start_transfer(pchannel_p, 1);
			break;
		case FINISH_XFER:
...
static int dma_proxy_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "dma_proxy module initialized;  BUFFER_COUNT = %d; BUFFER_SIZE = %d\n",  BUFFER_COUNT, BUFFER_SIZE);
	...
	if (rc) {
		printk(KERN_ERR "dma_proxy dma_proxy_probe: Can't initialize dma_proxy_tx\n");
//		return rc;
	}

	rc2 = create_channel(pdev, &channels[RX_CHANNEL], "dma_proxy_rx", DMA_DEV_TO_MEM);
	if (rc2) { 
		printk(KERN_ERR "dma_proxy dma_proxy_probe: Can't initialize dma_proxy_rx\n");
		if (rc2 && rc) {
			printk(KERN_ERR "dma_proxy dma_proxy_probe: No tx, no Rx, exiting..\n");
			return rc;
		}
	}
