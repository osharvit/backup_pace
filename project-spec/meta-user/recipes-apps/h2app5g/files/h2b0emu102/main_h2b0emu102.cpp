...
std::string g_fw_ver("0.2.359.c43adfa");
...
// threads
//======================================================
time_thread timethread("time", THREAD_TIME);
...
tcpserv_thread mpmonlstnthread("mpmonlstn",
                               THREAD_MPMON_LSTN,
                               g_listen_port + 1,
                               THREAD_MPMON_CONN,
...
hswi_thread hswilstnthread("hswilstn",
                           THREAD_HSWI_LSTN,
                           g_listen_port,
                           THREAD_HSWI_CONN,
                           {HSWI_CONNSOCK_0, HSWI_CONNSOCK_1, HSWI_CONNSOCK_2});

sp_thread spthread("sp", THREAD_SP, p1h2top);
il_thread ilthread("il", THREAD_IL, p1h2top);
re_thread h2appthread("re", THREAD_RE);
...
void init_devices()
{
    ...
    // major version
    assert(fpgaversion.u16le[1].u16 == 1);
    // minor version
    //assert(fpgaversion.u16le[0].u16 >= 31);
