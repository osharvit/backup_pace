    ...
    if(p1arg != NULL){
        ...
        switch(p1msg->hdr.msgid) {
            ...
            case ITC_PLAYBACK_REQ_ID: {
                ...
                //---------------------------------------------------------------
                // prepare_tmplayback(): TM playback
                //---------------------------------------------------------------
                //  <sp_idle_state>
                //      <prepare_tmplayback>
                //          <gen_axidma_tmdesc>
                //              <gen_valid_tdd_symb_list/>
                //          </gen_axidma_tmdesc>
                //      </prepare_tmplayback>
                //  </sp_idle_state>
                //---------------------------------------------------------------
    ...
    for(int plbk = 0; plbk < tmplaybackreq.num_playbackobj; plbk++){
        ...
        for(int ns = 0; ns < p1playback->numsection; ns++){
            ...
            std::cout << "[" << ns << "]" <<
                ...
                " fft_size=" << (int)p1section->fftsize <<
...
std::vector<int> sp_thread::gen_valid_tdd_symb_list(const playback_struct *p1playback)
{
    // p1playback->tdd_ul_dl_config, p1playback->tdd_ssf_config, p1playback->ul_only
    std::vector<int> symblist;
    assert(p1playback->numsection > 0);
    int num_symb_per_sf = 14;   // default for numerology = 0
    int numerology = p1playback->section[0].scs;

    //TRACE0OBJ() << "tdd_ul_dl_config="  << p1playback->tdd_ul_dl_config << std::endl;
    //std::cout << "tdd_ssf_config="      << p1playback->tdd_ssf_config << std::endl;
    //std::cout << "dl_ul_periodicity_x=" << p1playback->dl_ul_periodicity_x << std::endl;
    //std::cout << "nrofdl_slots="        << p1playback->nrofdl_slots << std::endl;
    //std::cout << "nrofdl_symbols="      << p1playback->nrofdl_symbols << std::endl;
    //std::cout << "nroful_slots="        << p1playback->nroful_slots << std::endl;
    //std::cout << "nroful_symbols="      << p1playback->nroful_symbols << std::endl;
    ...
    switch(numerology){
        ...
        default:{
            TRACE0OBJ() << "not yet implemented for the numerology=" << numerology << std::endl;
        }
    }

    if(p1playback->dl_ul_periodicity_x > 0){        // P = [2.5, 5, 10, 0.5, 0.625, 1.25] ms
        int period = (10000 / p1playback->dl_ul_periodicity_x);
        // period [4, 2, 1, 20, 16, 8] = 10 / [2.5, 5, 10, 0.5, 0.625, 1.25]
        assert((period == 4) || (period == 2) || (period == 1) || (period == 20) || (period == 16) || (period == 8));

        int symbidx = 0;
        for (int pp = 0; pp < period; pp++){
            if(p1playback->ul_only == 0){      // downlink as the default
                symbidx = (int)(pp*(num_symb_per_sf*10.0/period));
                //TRACE0OBJ() << "symbidx=" << symbidx << std::endl;
                for(int ll = 0; ll < p1playback->nrofdl_slots; ll++){
                    for(int mm = 0; mm < 14; mm++){ // full slot
                        symblist.push_back(symbidx);
                        symbidx ++;
                    }
                }
                {
                    for(int mm = 0; mm < p1playback->nrofdl_symbols; mm++){ // partial slot
                        //TRACE0OBJ() << "symbidx=" << symbidx << std::endl;
                        symblist.push_back(symbidx);
                        symbidx ++;
                    }
            ...
            else if(p1playback->ul_only == 1){ // uplink
                symbidx = (int)((pp + 1)*(num_symb_per_sf*10.0/period)) - (p1playback->nroful_symbols + p1playback->nroful_slots*14);
                TRACE0OBJ() << "symbidx=" << symbidx << std::endl;
                {
                    for(int mm = 0; mm < p1playback->nroful_symbols; mm++){ // partial slot
                        symblist.push_back(symbidx);
                        symbidx ++;
                    }
                } 
                for(int ll = 0; ll < p1playback->nroful_slots; ll++){
                    for(int mm = 0; mm < 14; mm++){ // 14 symbols for a slot
                        symblist.push_back(symbidx);
                        symbidx ++;
                    }
                }
    ...
    else if(p1playback->tdd_ul_dl_config == 2){
        int num_symb = 14;

        switch(p1playback->tdd_ssf_config){
            ...
            case    7: { // 10 OFDM symbols for Dw and 2 OFDM symbols for Up
                ...
                if(p1playback->ul_only == 0){    // downlink
                    for(sf = 0; sf < 1; sf++){ // DL
                        for(int symb = 0; symb < num_symb; symb++){
                            symblist.push_back(sf*num_symb + symb);
                        }
            ...
            default:{
                TRACE0OBJ() << "not yet implemented for the tdd_ul_dl_config=" << p1playback->tdd_ul_dl_config << std::endl;
            }
        }
    }
    else{   // FDD
        symblist.clear();
        for(int kk = 0; kk < (num_symb_per_sf*10); kk++){
                                    ...
                                    const std::string &tmwav_path)
{
...
#else
    int fftsize = p1playback->section[0].fftsize;
    double samplerateMHz = p1playback->section[0].fs;
#endif
    ...
    switch(u){
        case 0:{    // numerology=0 for 15KHz
            ...
            //      <a> = {fftsize + 144*(2**0)*30.72/30.72}/30.72 = 71.354 us
            //      or
            //      <a> = {fftsize + 144*(2**0)*61.44/30.72}/61.44 = 71.354 us
            ...
            num_symb_per_sf = 14;
#if 0
            segmentgap = (int)((fftsize + 144*pow<double>(2.0, -u)*30.72/30.72)/30.72*DMA_TS_CLK_IN_MHZ/num_segments);
#else
            segmentgap = (int)((fftsize + 144*pow<double>(2.0, -u)*samplerateMHz/30.72)/samplerateMHz*DMA_TS_CLK_IN_MHZ/num_segments);
#endif
            break;
        }
        case 1:{    // numerology=1 for 30KHz
            // short symbol duration for either NT50 or NR100:
            //      <a> = {fftsize + 144*(2**-1)*61.44/30.72}/61.44 = 35.677 us
            //      or
            //      <a> = {fftsize + 144*(2**-1)*DMA_TS_CLK_IN_MHZ/30.72}/DMA_TS_CLK_IN_MHZ = 35.677 us
            // number of symbols in DMA_TS_CLK_IN_MHZ Msps which is the AXIDMA timer clock:
            ...
            num_symb_per_sf = 28;
#if 0
            segmentgap = (int)((fftsize + 144*pow<double>(2.0, -u)*DMA_TS_CLK_IN_MHZ/30.72)/30.72*DMA_TS_CLK_IN_MHZ/num_segments);
#else
            segmentgap = (int)((fftsize + 144*pow<double>(2.0, -u)*samplerateMHz/30.72)/samplerateMHz*DMA_TS_CLK_IN_MHZ/num_segments);
#endif
            break;
    ...
    switch(p1playback->section[0].frame_structure){
        case 0:{    // FS 8
            ...
            if(u <= 2){    // numerology:0 for 15 KHz, 1 for 30 KHz, 2 for 60 KHz
                int cp_other = (int)(144*pow<double>(2.0, -u)*(double)p1playback->section[0].fs_x/30720.0);
                int cp_first = cp_other + (int)(16*(double)p1playback->section[0].fs_x/30720.0);
                TRACE0OBJ() << "cp=" << cp_first << "," << cp_other << std::endl;
                int kk = 0;
                {
                    // copy the 1st section
                    symbinfolist[kk] = p1playback->section[0];
                    symbinfolist[kk].cp_length = cp_first; 
                }
                for(kk = 1; kk < half_symb_per_sf; kk++){
                    // copy the 1st section
                    symbinfolist[kk] = p1playback->section[0];
                    symbinfolist[kk].cp_length = cp_other; 
                }
                {
                    // copy the 1st section
                    symbinfolist[kk] = p1playback->section[0];
    ...
    //---------------------------------------------------------------
    // gen_valid_tdd_symb_list(): generate TDD symbol table
    //---------------------------------------------------------------
    //  <sp_idle_state>
    //      <prepare_tmplayback>
    //          <gen_axidma_tmdesc>
    ...
    //  </sp_idle_state>
    //---------------------------------------------------------------
    std::vector<int> validsymblist = this->gen_valid_tdd_symb_list(p1playback);
    if((validsymblist.size() != 140) && (validsymblist.size() != 280)) {
        // print the validsymblist for gated(dtx) mode
        TRACE0OBJ() << "validsymbsize=" << validsymblist.size() << std::endl;
    ...
    for(int kk = 0; kk < NUM_SF; kk++){
        for(int ll = 0; ll < num_symb_per_sf; ll++){
            ...
            //TRACE0OBJ() << "kk=" << kk << ", ll=" << ll << ", osr=" << (int)symbinfolist[ll].osr << ", numsamples=" << numsamples << std::endl;

            symbpos = kk*num_symb_per_sf + ll;

            bool isgated = (p1playback->off_duty_mode == 1);  // 0: "nongated", 1: "gated"
            ...
            TRACE0OBJ()
                ...
                << std::endl;
#endif
            // if nongated mode or valid symbol
            if((isgated == false) || isvalidsymbol){
                ...
                for(int ss = 0; ss < num_segments; ss++){
                    ...
                    //TRACE0OBJ() << "kk=" << kk << ", ll=" << ll << ", ss=" << ss << ", wordsize=" << seg_wordsize << std::endl;
                    ...
                    //TRACE0OBJ() << "timestamp=" << descriptor.USER_APP_TIMESTAMP << std::endl;
                    if(playback_idx > 0){
